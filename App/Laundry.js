...
export default class Laundry extends Component<{}> {
    ...
    textChange(event) {
        ...
        let filteredData = rooms.filter(room => room.includes(event));
        this.setState({dataSource: ds.cloneWithRows(filteredData)});
  ...
  render() {
    ...
    return (
      ...
      renderRow={(data) => <TouchableOpacity onPress={() => navigate('Room',{name: data, key: this.state.rooms[data]})} style={styles.section}><View><Text style={styles.label}>{data}</Text></View></TouchableOpacity>}
